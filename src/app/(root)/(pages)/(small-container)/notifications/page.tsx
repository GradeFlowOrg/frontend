...
const Page = () => {
  ...
  const renderNotificationCard = (item: notificationItemsInterface) => (
    <article
      key={item.id}
      onClick={() => handleOpenNotification(item)}
      className="cursor-pointer rounded-[24px] border border-slate-200/80 bg-white/90 p-4 shadow-[0_12px_32px_rgba(15,23,42,0.08)] transition hover:border-slate-300 hover:shadow-[0_14px_36px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-[#1f1f1f] dark:hover:border-white/15"
    >
    ...
    <div className="flex min-h-[60vh] w-full flex-col">
      {items.length > 0 ? (
        <section className="space-y-3">
          {unreadMessages.map(renderNotificationCard)}
          {readMessages.map(renderNotificationCard)}
        </section>
      ) : (
